...
import { approvePartyReport, fetchPartyReportDetail, fetchPartyReports, rejectPartyReport } from '../api/party'
...
async function loadData() {
  loading.value = true
  try {
    rows.value = await fetchPartyReports({ status: statusFilter.value })
  } catch (error) {
...
async function openDetail(id) {
  ...
  reviewForm.comment = ''
  try {
    detail.value = await fetchPartyReportDetail(id)
  } catch (error) {
...
async function doApprove() {
  ...
  try {
    await approvePartyReport(detail.value.id, comment)
...
async function doReject() {
  ...
  reviewing.value = true
  try {
    await rejectPartyReport(detail.value.id, comment)
...
async function downloadFile(fileId) {
  ...
  try {
    const res = await axios.get(`${baseURL}/api/files/${fileId}/download`, {
      responseType: 'blob',
    ...
    ElMessage.error((error && error.message) || '下载失败')
  }
}
...
<template>
  <el-card shadow="never">
    ...
    <el-table :data="rows" v-loading="loading" stripe empty-text="暂无思想汇报">
      ...
      <el-table-column label="操作" width="120" fixed="right">
        <template #default="{ row }">
          <el-button type="primary" link @click="openDetail(row.id)">{{ statusFilter === 0 ? '审核' : '查看' }}</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-drawer v-model="drawerOpen" size="520px" :with-header="true" title="思想汇报详情" @close="closeDrawer">
      <div v-loading="detailLoading">
        <template v-if="detail">
          <el-descriptions :column="1" border>
            <el-descriptions-item label="学生">{{ detail.realName }}（{{ detail.studentNo }}）</el-descriptions-item>
            <el-descriptions-item label="阶段">{{ detail.stageCode || '-' }}</el-descriptions-item>
            <el-descriptions-item label="标题">{{ detail.title }}</el-descriptions-item>
            <el-descriptions-item label="内容">
              <div class="content">{{ detail.content || '-' }}</div>
            </el-descriptions-item>
            <el-descriptions-item label="附件">
              <el-button v-if="detail.fileId" size="small" @click="downloadFile(detail.fileId)">下载附件</el-button>
              <span v-else>-</span>
            </el-descriptions-item>
